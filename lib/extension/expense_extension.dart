...
extension ExpensesExtension on List<Expenses> {
  List filterByPeriod(Period period, int periodIndex) {
    ...
    return [expense, startDate, endDate];
  }
  ...
  groupWeekly() {
    final Map<String, List<Expenses>> grouped = {
      'Monday': [],
      'Tuesday': [],
      'Wednesday': [],
      'Thursday': [],
  ...
  groupMonthly(DateTime startDate) {
    final numsofDay = DateTime(startDate.year, startDate.month + 1, 0).day;
    final Map<int, List<Expenses>> grouped =
        List.generate(numsofDay, (index) => <Expenses>[]).asMap();
    forEach((element) {
  ...
  groupYearly() {
    final Map<int, List<Expenses>> grouped =
        List.generate(12, (index) => <Expenses>[]).asMap();
    forEach((element) {
